-- Add best-available native evidence fallback fields for Finding Detail.
--
-- Most CIS findings have a native per-control detail CSV. Summary-level rows do
-- not. For those rows, the Finding Detail page should offer the native HTML
-- summary instead of an empty detail CSV link.
