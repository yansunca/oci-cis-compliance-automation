-- Remove retired demo workflow objects from an already-provisioned ADB.
--
-- This keeps the current customer demo read-only and audit-focused. It drops
-- only workflow/action objects; scan runs, CIS findings, products, raw records,
-- report artifacts, and evidence links remain intact.
...
SELECT
    ...
    COALESCE(
        evidence_choices.best_download_url,
...
BEGIN
    FOR object_name IN (
        SELECT 'V_CIS_APEX_ACTION_AUDIT' AS name, 'VIEW' AS object_type FROM dual
        UNION ALL SELECT 'V_CIS_APEX_FINDING_ACTION_SUMMARY', 'VIEW' FROM dual
        UNION ALL SELECT 'FINDING_LIFECYCLE', 'PACKAGE' FROM dual
        UNION ALL SELECT 'APEX_WORKFLOW_SECURITY', 'PACKAGE' FROM dual
        UNION ALL SELECT 'APEX_WORKFLOW_OPERATOR', 'TABLE' FROM dual
        UNION ALL SELECT 'FINDING_ACTION_AUDIT', 'TABLE' FROM dual
        UNION ALL SELECT 'FINDING_SUPPRESSION', 'TABLE' FROM dual
        UNION ALL SELECT 'FINDING_RISK_ACCEPTANCE', 'TABLE' FROM dual
        UNION ALL SELECT 'FINDING_COMMENT', 'TABLE' FROM dual
        UNION ALL SELECT 'FINDING_ASSIGNMENT', 'TABLE' FROM dual
    ) LOOP
        BEGIN
            EXECUTE IMMEDIATE 'DROP ' || object_name.object_type || ' ' || object_name.name;
        EXCEPTION
            WHEN OTHERS THEN
